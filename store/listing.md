...
Account-level, not per-app: Play shows this above the list of everything `ltns`
publishes. Reached from Play Console under **Grow users → Store presence →
Developer page**. Every field marked `*` in the Console blocks the save, which is
why an empty promo text shows up as "some languages have errors" rather than as a
missing field.

| Field | Value | Spec Play enforces | Ours |
| --- | --- | --- | --- |
...
generated by `npm run art`, and the developer pair is written as JPEG for the same
reason the feature graphic is - a canvas `toDataURL` PNG always carries an alpha
channel.

Two things drove the artwork rather than taste:

- Play renders the developer icon as a **circle**, so the mark sits inside a
  centred disc and the corners carry nothing but background.
- The header is cropped hard on narrow screens and the developer name and icon
